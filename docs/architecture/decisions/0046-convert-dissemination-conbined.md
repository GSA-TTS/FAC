# 46. Convert dissemination_combined MV to a real table
...
* We can't make the data reliably available to users as an API because of the nightly downtime, but it is useful for devs
  * Because of this, v1.1.1 is currently the only one with the combined data available, but we have to keep updating it as we create new API versions. This is cumbersome to maintain and test.
