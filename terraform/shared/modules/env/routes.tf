...
data "cloudfoundry_app" "clients" {
  ...
  depends_on = [module.clamav, module.file_scanner_clamav, cloudfoundry_app.postgrest, module.https-proxy]
}
...
resource "cloudfoundry_network_policy" "app-to-proxy-network-policy" {
  provider = cloudfoundry-community
  policy {
