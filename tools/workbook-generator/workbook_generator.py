...
class WorkbookGenerator:
    def set_single_cell_range(self, wb, range_name, value):
    ...
    def set_range(self, wb, range_name, values, default=None, type=str):
        the_range = wb.defined_names[range_name]
        ...
        col = self.col_to_ndx(start_cell[0])
    ...
    def set_uei(self, wb, dbkey):
        g = Gen.select().where(Gen.dbkey == dbkey).get()
        self.set_single_cell_range(wb, 'auditee_uei', g.uei)
        return g

    def map_simple_columns(self, wb, mappings, values):
    ...
    def dbkey_to_report_id(self, dbkey):
    ...
    def generate_dissemination_test_table(self, api_endpoint, dbkey, mappings, objects):
        table = {
            'rows': list(),
            'singletons': dict()
        }
        table['endpoint'] = api_endpoint
        table['report_id'] = self.dbkey_to_report_id(dbkey)
        for o in objects:
    ...
    #
    ##########################################
    def generate_findings(self, dbkey, outdir):
        ...
        if (cfdas != None) and (findings != None):
            ...
            if len(e2a) != 0:
                ...
                if len(award_references) != 0:
                    ...
                    self.set_range(wb, 'award_reference', award_references)

        wb.save(os.path.join(outdir, f'findings-{dbkey}.xlsx'))

        table = self.generate_dissemination_test_table('findings', dbkey, mappings, findings)
    ...
    #
    ##########################################
    def generate_federal_awards(self, dbkey, outdir):
        ...
        self.map_simple_columns(wb, mappings, cfdas)

        # Map things with transformations
        prefixes = map(lambda v: (v.cfda).split('.')[0], cfdas)
        extensions = map(lambda v: (v.cfda).split('.')[1], cfdas)
        self.set_range(wb, 'federal_agency_prefix', prefixes)
        self.set_range(wb, 'three_digit_extension', extensions)
        ...
        self.set_range(wb, 'passthrough_name', passthrough_names)
        self.set_range(wb, 'passthrough_identifying_number', passthrough_ids)

        # Total amount expended must be calculated and inserted
        total = 0
        for cfda in cfdas:
            total += int(cfda.amount)
        self.set_single_cell_range(wb, 'total_amount_expended', total)

        wb.save(os.path.join(outdir, f'federal-awards-{dbkey}.xlsx'))

        table = self.generate_dissemination_test_table('federal_awards', dbkey, mappings, cfdas)
        # prefix
    ...
    #
    ##########################################
    def generate_findings_text(self, dbkey, outdir):
        ...
        self.map_simple_columns(wb, mappings, ftexts)
        wb.save(os.path.join(outdir, f'findings-text-{dbkey}.xlsx'))
        table = self.generate_dissemination_test_table('findings_text', dbkey, mappings, ftexts)
        table['singletons']['auditee_uei'] = g.uei
    ...
    #
    ##########################################
    def generate_additional_ueis(self, dbkey, outdir):
        ...
        self.map_simple_columns(wb, mappings, addl_ueis)
        wb.save(os.path.join(outdir, f'additional-ueis-{dbkey}.xlsx'))
        table = self.generate_dissemination_test_table('additional_ueis', dbkey, mappings, addl_ueis)
        table['singletons']['auditee_uei'] = g.uei
    ...
    #
    ##########################################
    def generate_notes_to_sefa(self, dbkey, outdir):
        ...
        self.set_single_cell_range(wb, 'accounting_policies', policies.content)
        # WARNING
        ...
        self.set_single_cell_range(wb, 'is_minimis_rate_used', is_used)
        self.set_single_cell_range(wb, 'rate_explained', rate.content)

        # Map the rest as notes.
        self.map_simple_columns(wb, mappings, notes)
        wb.save(os.path.join(outdir, f'notes-{dbkey}.xlsx'))

        table = self.generate_dissemination_test_table('notes_to_sefa', dbkey, mappings, notes)
    ...
    #
    ##########################################
    def generate_secondary_auditors(self, dbkey, outdir):
        ...
        self.map_simple_columns(wb, mappings, sec_cpas)
        wb.save(os.path.join(outdir, f'cpas-{dbkey}.xlsx'))

        table = self.generate_dissemination_test_table('secondary_auditors', dbkey, mappings, sec_cpas)
        table['singletons']['auditee_uei'] = g.uei
    ...
    #
    ##########################################
    def generate_captext(self, dbkey, outdir):
        ...
        self.map_simple_columns(wb, mappings, captexts)
        wb.save(os.path.join(outdir, f'captext-{dbkey}.xlsx'))

        table = self.generate_dissemination_test_table('cap_text', dbkey, mappings, captexts)
        table['singletons']['auditee_uei'] = g.uei

        return table


    def generate(self, dbkey, outdir):
        fat = self.generate_federal_awards(dbkey, outdir)
        ft = self.generate_findings(dbkey, outdir)
        ftt = self.generate_findings_text(dbkey, outdir)
        aut = self.generate_additional_ueis(dbkey, outdir)
        ntst = self.generate_notes_to_sefa(dbkey, outdir)
        sat = self.generate_secondary_auditors(dbkey, outdir)
        ctt = self.generate_captext(dbkey, outdir)
        tables = [sat, ctt, ntst, aut, ftt, ft, fat]
        with open(os.path.join(outdir, f'test-array-{dbkey}.json'), "w") as test_file:
