...
// console.log('init ', not_gaap_cb, not_gaap_cb.checked)
...
function attachEventHandlers() {
  ...
  FORM.addEventListener('submit', (e) => {
    ...
    if (!not_gaap_cb.checked) {
      const basis = document.querySelectorAll('input[name="sp_framework_basis"]');
      basis.forEach((basi) => {
        basi.checked = false;
      });
      const reqds = document.querySelectorAll('input[name="is_sp_framework_required"]');
      reqds.forEach((reqd) => {
        reqd.checked = false;
      });
      const opinions = document.querySelectorAll('input[name="sp_framework_opinions"]');
