...
var chart_data_submissions_with_findings = [
  {
    ...
    text: data_submissions_with_findings.map(
      (object) =>
        new Intl.NumberFormat('en-US', { maximumSignificantDigits: 3 }).format(
          object.total
        ) + '%'
...
data_auditee_risk_profile.forEach((object, index) =>
  chart_layout_auditee_risk_profile.annotations.push({
    text: object.year,
    showarrow: false,
    x: index % 2 ? 0.77 : 0.24,
    xref: 'paper',
    ...
    y:
      1.006 -
      Math.floor(index / 2) / Math.ceil(data_auditee_risk_profile.length / 2),
    yref: 'paper',
...
var chart_data_risk_profile_vs_findings = [
  // Trace one, % not low risk
  {
    ...
    text: data_risk_profile_vs_findings.map(
      (object) =>
        new Intl.NumberFormat('en-US', { maximumSignificantDigits: 3 }).format(
          object.not_low_risk
        ) + '%'
  ...
  {
    x: data_risk_profile_vs_findings.map((object) => object.year),
    y: data_risk_profile_vs_findings.map(
      (object) => object.audits_with_findings
    ),
    text: data_risk_profile_vs_findings.map(
      (object) =>
        new Intl.NumberFormat('en-US', { maximumSignificantDigits: 3 }).format(
