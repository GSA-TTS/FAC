...
class AuditeeInfoView(APIView):
    ...
    """
    DATA_WE_NEED = ['is_usa_based', 'user_provided_organization_type', 'met_spending_threshold']
    ...
    def post(self, request):
        ...
        if missing_fields:
            return Response({'next': reverse('eligibility'), 'errors': "We're missing important data, please try again."})

        if serializer.is_valid():
            next_step = reverse('access')

            # combine with expected eligibility info from session
            request.user.profile.entry_form_data = request.user.profile.entry_form_data | request.data
            request.user.profile.save()
...
class AccessView(APIView):
    DATA_WE_NEED = AuditeeInfoView.DATA_WE_NEED + ['uei', 'auditee_fiscal_period_start', 'auditee_fiscal_period_end', 'auditee_name']
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            sac = SingleAuditChecklist.objects.create(submitted_by=request.user, **entry_form_data)
