...
def test_federal_awards_named_ranges():
    for d in test_table:
        excel_ranges = extract_ranges_from_excel_def(
            os.path.join(os.getcwd(), excel_path, d["excel"])
        )
        schema_ranges = extract_ranges_from_schema(
            "FederalAwards", os.path.join(os.getcwd(), schema_path, d["schema"])
        )
