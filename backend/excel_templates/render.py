...
Range = NT("Range", "name,column,label_row,range_start_row,range_start,abs_range_start,full_range")
...
def process_single_cells(wb, ws, sheet):
    ...
    for o in sheet['single_cells']:
        ...
        sheet_cell_coordinate = f"{quote_sheetname(ws.title)}!{absolute_cell_coordinate}"
...
def make_range(r):
    ...
    full_range = f"{start_cell}:${column}${MAX_ROWS}"
    return Range(
        r['range_name'],
        column,
        title_row, 
...
def process_open_ranges(wb, ws, spec):
    for r in spec['open_ranges']:
        ...
        print(f"Open Range: {r['title']}, {coords.column}, {coords.abs_range_start}, {cell_reference}")
...
def configure_validation(ws, coords : Range, r):
    ...
    if ("validation" in r) and (r["validation"] != {}):
        ...
        if "formula1" in v:
            ...
            dv.formula1 = dv.formula1.replace("FIRSTCELLREF", f"${coords.column}{coords.range_start_row}")
        if "operator" in v:
