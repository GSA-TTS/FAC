...
class TestTrackInvalidRecords(SimpleTestCase):
    ...
    def test_with_changes(self, mock_timezone, mock_invalid_audit_record):
        ...
        InvalidRecord.reset()
        InvalidRecord.append_general_changes(["General change"])
        InvalidRecord.append_finding_changes(["Finding change"])
        InvalidRecord.append_note_changes(["Note change"])
        InvalidRecord.append_federal_awards_changes(["Federal award change"])
        InvalidRecord.append_secondary_auditor_changes(["Secondary auditor change"])
        ...
        self.assertEqual(
            mock_invalid_audit_record_instance.general, [["General change"]]
        )
        self.assertEqual(
            mock_invalid_audit_record_instance.finding, [["Finding change"]]
        )
        self.assertEqual(mock_invalid_audit_record_instance.note, [["Note change"]])
        self.assertEqual(
            mock_invalid_audit_record_instance.federal_award, [["Federal award change"]]
        )
        self.assertEqual(
            mock_invalid_audit_record_instance.secondary_auditor,
            [["Secondary auditor change"]],
        )

        mock_invalid_audit_record_instance.save.assert_called_once()
        self.assertEqual(InvalidRecord.change, InvalidRecord.DEFAULT)
