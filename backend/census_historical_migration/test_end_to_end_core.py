# FIXME: Commenting out this test because workbook_builder.py uses unmaintained
# `fs` module, which tries to use setuptools's removed `pkg_resources`. This
# breaks our unit tests. If this is needed again, replace `fs`/`MemoryFS` with
# something else.
