...
export function testValidGeneralInfo() {
	...
	cy.get('#ein').clear().type('546000173');
	...
	cy.get('#auditee_zip').clear().type('23219');
	...
	cy.get('#auditee_phone').clear().type('5558675309');
	cy.get('#auditee_email').clear().type('va@test.com');

	// Auditor information
	cy.get('#auditor_ein').clear().type('987654321');
	...
	cy.get('#auditor_zip').clear().type('20121');

	// Auditor contact information
	cy.get('#auditor_contact_name').type('Jane Doe');
	cy.get('#auditor_contact_title').type('Auditor');
	cy.get('#auditor_phone').clear().type('5555555555');
	cy.get('#auditor_email').clear().type('qualified.human.accountant@auditor.com');
