...
class DisseminationTrendAnalytics:
    ...
    def _get_records_by_years(self):
        ...
        return General.objects.filter(
            is_public=True,
            fac_accepted_date__year__in=self.years
        )
    ...
    def total_award_volume(self):
        ...
        for year in self.years:
            out.append({
                "year": year,
                "total": self.awards
                    .filter(report_id__fac_accepted_date__year=year)
    ...
    def total_findings(self):
        ...
        for year in self.years:
            out.append({
                "year": year,
                "total": self.findings.filter(report_id__fac_accepted_date__year=year).count()
            })
        return out
    ...
    def submissions_with_findings(self):
        ...
        for year in self.years:
            ...
            findings_count = self._get_records_by_year(year).exclude(finding=None).count()
            out.append({
                "year": year,
                "total": (findings_count / audits_count) * 100
            })
        return out
    ...
    def auditee_risk_profile(self):
        ...
        for year in self.years:
            ...
            low_risk_count = self._get_records_by_year(year).filter(is_low_risk_auditee='Yes').count()
            not_low_risk_count = self._get_records_by_year(year).filter(is_low_risk_auditee='No').count()
