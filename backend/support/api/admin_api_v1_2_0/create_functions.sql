...
CREATE OR REPLACE FUNCTION admin_api_v1_2_0_functions.get_header(item text) RETURNS text
    AS $get_header$
    declare res text;
   	begin
...
create or replace function admin_api_v1_2_0_functions.get_api_key_uuid() returns TEXT
as $gaku$
declare uuid text;
begin
	select admin_api_v1_2_0_functions.get_header('x-api-user-id') into uuid;
	return uuid;
end;
...
create or replace function admin_api_v1_2_0_functions.log_admin_api_event(event TEXT, meta JSON)
returns boolean
as $log_admin_api_event$
DECLARE
    uuid_header text;
BEGIN
    SELECT admin_api_v1_2_0_functions.get_api_key_uuid() INTO uuid_header;
...
create or replace function admin_api_v1_2_0_functions.has_admin_data_access(perm TEXT) returns boolean
...
-- content-profile: admin_api_v1_2_0
...
-- }
create or replace function admin_api_v1_2_0.add_tribal_access_email(params JSON) 
returns BOOLEAN
...
BEGIN
    ...
    IF admin_api_v1_2_0_functions.has_admin_data_access('CREATE')
    THEN
        ...
        THEN
            ...
            RETURN admin_api_v1_2_0_functions.log_admin_api_event('tribal-access-email-added', 
...
--     ]
-- }
CREATE OR REPLACE FUNCTION admin_api_v1_2_0.add_tribal_access_emails(params JSON) 
returns BOOLEAN
...
BEGIN
    ...
    THEN 
        ...
        LOOP
            ...
            PERFORM admin_api_v1_2_0.add_tribal_access_email(json_build_object('email', em.ele)::JSON);
        END LOOP;
        RETURN 1;
    END IF;
    RETURN 0;
...
--     "email": "darth.vader@deathstar.org"
-- }
CREATE OR REPLACE FUNCTION admin_api_v1_2_0.remove_tribal_access_email(params JSON) 
returns BOOLEAN
...
BEGIN
    ...
    IF admin_api_v1_2_0_functions.has_admin_data_access('DELETE')
    THEN 
        ...
        IF affected_rows > 0
        THEN
            RETURN admin_api_v1_2_0_functions.log_admin_api_event('tribal-access-email-removed', 
                                                        json_build_object('email', LOWER(params->>'email')));
        ELSE
            RETURN 0;
        END IF;
...
--     ]
-- }
CREATE OR REPLACE FUNCTION admin_api_v1_2_0.remove_tribal_access_emails(params JSON) 
returns BOOLEAN
...
BEGIN
    ...
    THEN 
        FOR em IN (SELECT json_array_elements_text((params->>'emails')::JSON) ele)
        LOOP
            PERFORM admin_api_v1_2_0.remove_tribal_access_email(json_build_object('email', em.ele)::JSON);
        END LOOP;
        RETURN 1;
    END IF;
    RETURN 0;
...
CREATE OR REPLACE FUNCTION admin_api_v1_2_0.add_tribal_api_key_access(params JSON) 
RETURNS JSON
...
BEGIN
    ...
    IF admin_api_v1_2_0_functions.has_admin_data_access('CREATE') THEN
...
CREATE OR REPLACE FUNCTION admin_api_v1_2_0.remove_tribal_api_key_access(params JSON) 
RETURNS JSON
...
BEGIN
    ...
    IF admin_api_v1_2_0_functions.has_admin_data_access('DELETE') THEN
...
CREATE OR REPLACE FUNCTION admin_api_v1_2_0.request_file_access(
    report_id TEXT
...
BEGIN
    ...
    SELECT admin_api_v1_2_0_functions.get_api_key_uuid() INTO v_uuid_header;
