...
def compute_cog_over(
    ...
    if not federal_awards:
        ...
        return (None, None)
    awards = federal_awards["awards"]
    total_amount_expended = federal_awards["total_amount_expended"]
...
def determine_hist_agency(ein, uei, audit_year):
    ...
    (gen_count, total_amount_expended, report_id_base_year) = get_base_audit(
        ein, uei, base_year
    )
    if gen_count != 1:
        return None
...
def get_base_cfdas(report_id):
    ...
    baseline_cfdas = []
    for row in cfdas:
        baseline_cfdas.append(
            (
                row["program"]["federal_agency_prefix"],
                row["program"]["amount_expended"],
                row["direct_or_indirect_award"]["is_direct"],
            )
        )
