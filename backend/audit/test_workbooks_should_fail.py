...
def map_file_to_extractor_validator(filename):
    file_mapping = {
        ...
        "audit-findings-text-": (extract_audit_findings_text, validate_findings_text_json),
        "audit-findings-": (extract_audit_findings, validate_findings_uniform_guidance_json),
        "corrective-action-plan": (extract_corrective_action_plan, validate_corrective_action_plan_json),
        ...
        "secondary-auditors": (extract_secondary_auditors, validate_secondary_auditors_json)
...
class FailingWorkbooks(SimpleTestCase):
    ...
    def test_passing_workbooks(self):
        workbook_sets = reduce(os.path.join, ["audit", "fixtures", "workbooks", "should_fail"])
        workbook_count = 0
        failure_count = 0
        for (dirpath, dirnames, _) in os.walk(workbook_sets):
            for workbook_set in dirnames:
                print("Walking ", workbook_set)
                for (wb_path, _, wb_files) in os.walk(os.path.join(dirpath, workbook_set)):
